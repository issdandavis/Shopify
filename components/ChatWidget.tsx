...
export const ChatWidget: React.FC<ChatWidgetProps> = ({ onNavigate }) => {
    ...
    const [messages, setMessages] = useState<Message[]>([
        { role: 'model', text: 'Hi! I can help you with AliExpress sourcing, Canva designs, or navigating your projects. Try "show my design tasks"!' }
    ]);
    ...
    return (
        <div className="fixed bottom-4 right-4 z-50 flex flex-col items-end">
            {isOpen && (
                <div className="mb-4 w-96 h-[500px] bg-white rounded-2xl shadow-2xl border border-gray-200 flex flex-col overflow-hidden animate-fade-in-up">
                    <div className="bg-gray-900 text-white p-4 flex items-center justify-between">
                        <div className="flex items-center gap-2">
                            <SparklesIcon className="w-5 h-5 text-green-500" />
                            <h3 className="font-bold text-sm">App Architect AI</h3>
                        </div>
                        <button onClick={() => { setIsOpen(false); stopSpeech(); setIsSpeaking(false); }}><XIcon className="w-5 h-5" /></button>
                    </div>
                                    ...
                                    {msg.sources && msg.sources.length > 0 && (
                                        <div className="mt-3 pt-3 border-t border-gray-100 flex flex-wrap gap-2">
                                            {msg.sources.map((src, sIdx) => (
                                                src.web ? (
                                                    <a key={sIdx} href={src.web.uri} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-[10px] bg-gray-50 text-green-600 px-2 py-1 rounded-full border hover:bg-green-50 transition-colors">
                    ...
                    <form onSubmit={handleSendMessage} className="p-3 border-t flex gap-2 bg-white">
                        <button type="button" onClick={toggleVoice} className={`p-2 rounded-full ${isListening ? 'bg-red-50 text-red-500 animate-pulse' : 'text-gray-400 hover:text-green-600'}`}>
                          <MicrophoneIcon className="w-5 h-5" />
                        </button>
                        <input type="text" value={inputValue} onChange={(e) => setInputValue(e.target.value)} placeholder="e.g. 'Show me design tasks'" className="flex-1 bg-gray-100 rounded-full py-2 px-4 outline-none text-sm" />
                        <button type="submit" className="p-2 bg-green-600 text-white rounded-full hover:bg-green-700 transition-colors shadow-md"><PaperAirplaneIcon className="w-5 h-5" /></button>
                    </form>
                </div>
            )}
